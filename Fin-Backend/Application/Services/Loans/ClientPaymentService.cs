...
namespace FinTech.Core.Application.Services
{
    public class ClientPaymentService : IClientPaymentService
    {
        ...
        public async Task<IEnumerable<SavedPayee>> GetSavedPayeesRawAsync(Guid customerId)
        {
            try
            {
        ...
        public async Task<SavedPayee> SavePayeeRawAsync(SavePayeeDto payeeDto, Guid customerId)
        {
            try
            {
                // Validate biller exists
        ...
        public async Task<IEnumerable<RecurringPayment>> GetRecurringPaymentsRawAsync(Guid customerId)
        {
            try
            {
        ...
        public async Task<bool> CancelRecurringPaymentRawAsync(Guid recurringPaymentId, Guid customerId)
        {
            try
            {
        ...
        // Interface wrappers (implement IClientPaymentService)
        ...
        {
            var source = await _dbContext.DepositAccounts.FindAsync(fromAccountId);
            if (source == null) throw new KeyNotFoundException("Source account not found");

            var dto = new FinTech.Core.Application.DTOs.ClientPortal.BillPaymentDto
            {
            ...
            return await PayBillAsync(dto, customerId);
        }
        ...
        {
            var source = await _dbContext.DepositAccounts.FindAsync(fromAccountId);
            var dest = await _dbContext.DepositAccounts.FindAsync(toAccountId);
            if (source == null || dest == null) throw new KeyNotFoundException("Account not found");

            var dto = new FinTech.Core.Application.DTOs.ClientPortal.FundTransferDto
            {
                SourceAccountNumber = source.AccountNumber,
            ...
            var tr = await TransferFundsAsync(dto, customerId);
            // Map TransferResult to PaymentResult expected by the interface
            return new PaymentResult
            {
                Success = tr.Success,
        ...
        public async Task<PaymentResult> ProcessExternalTransferAsync(Guid customerId, Guid fromAccountId, Guid beneficiaryId, decimal amount, string reference, bool isRecurring = false)
        {
            ...
            // For external transfers, beneficiaryId may map to an external beneficiary record; fallback to using beneficiaryId as account number if missing
            var dto = new FinTech.Core.Application.DTOs.ClientPortal.FundTransferDto
            {
                SourceAccountNumber = source.AccountNumber,
                DestinationAccountNumber = beneficiaryId.ToString(),
                Amount = amount,
        ...
        public async Task<FinTech.Core.Application.Common.Models.BaseResponse<List<FinTech.Core.Application.DTOs.ClientPortal.BillPaymentDto>>> GetRecentBillPaymentsAsync(Guid customerId, int count = 5)
        {
            var items = await _dbContext.BillPayments
                .Where(p => p.CustomerId == customerId)
                .OrderByDescending(p => p.PaymentDate)
                .Take(count)
                .ToListAsync();

            var dtos = items.Select(p => new FinTech.Core.Application.DTOs.ClientPortal.BillPaymentDto
            {
                SourceAccountNumber = _dbContext.DepositAccounts.Find(p.AccountId)?.AccountNumber ?? string.Empty,
                BillerId = p.BillerId,
                CustomerReferenceNumber = p.CustomerReferenceNumber,
                Amount = p.Amount,
                Description = p.Description
            }).ToList();

            return FinTech.Core.Application.Common.Models.BaseResponse<List<FinTech.Core.Application.DTOs.ClientPortal.BillPaymentDto>>.SuccessResponse(dtos);
        }

        public async Task<FinTech.Core.Application.Common.Models.BaseResponse<List<FinTech.Core.Application.DTOs.ClientPortal.TransferDto>>> GetRecentTransfersAsync(Guid customerId, int count = 5)
        {
            var txs = await _dbContext.DepositTransactions
                .Where(t => _dbContext.DepositAccounts.Any(a => a.Id == t.AccountId && a.CustomerId == customerId))
                .OrderByDescending(t => t.TransactionDate)
                .Take(count)
                .ToListAsync();

            var dtos = txs.Select(t => new FinTech.Core.Application.DTOs.ClientPortal.TransferDto
            {
            ...
            return FinTech.Core.Application.Common.Models.BaseResponse<List<FinTech.Core.Application.DTOs.ClientPortal.TransferDto>>.SuccessResponse(dtos);
        }

        // Recurring payments and saved payees - minimal implementations to satisfy interface
        public Task<FinTech.Core.Application.Common.Models.BaseResponse<List<FinTech.Core.Application.DTOs.ClientPortal.RecurringPaymentDto>>> GetRecurringPaymentsAsync(Guid customerId)
        {
            return Task.FromResult(FinTech.Core.Application.Common.Models.BaseResponse<List<FinTech.Core.Application.DTOs.ClientPortal.RecurringPaymentDto>>.Failure("Not implemented"));
        }

        public Task<FinTech.Core.Application.Common.Models.BaseResponse<FinTech.Core.Application.DTOs.ClientPortal.RecurringPaymentDto>> GetRecurringPaymentDetailsAsync(Guid customerId, Guid paymentId)
        {
            return Task.FromResult(FinTech.Core.Application.Common.Models.BaseResponse<FinTech.Core.Application.DTOs.ClientPortal.RecurringPaymentDto>.Failure("Not implemented"));
        }

        public Task<FinTech.Core.Application.Common.Models.BaseResponse<FinTech.Core.Application.DTOs.ClientPortal.RecurringPaymentDto>> CreateRecurringPaymentAsync(Guid customerId, FinTech.Core.Application.DTOs.ClientPortal.RecurringPaymentCreateDto paymentDto)
        {
            return Task.FromResult(FinTech.Core.Application.Common.Models.BaseResponse<FinTech.Core.Application.DTOs.ClientPortal.RecurringPaymentDto>.Failure("Not implemented"));
        }

        public Task<FinTech.Core.Application.Common.Models.BaseResponse<FinTech.Core.Application.DTOs.ClientPortal.RecurringPaymentDto>> UpdateRecurringPaymentAsync(Guid customerId, Guid paymentId, FinTech.Core.Application.DTOs.ClientPortal.RecurringPaymentUpdateDto paymentDto)
        {
            return Task.FromResult(FinTech.Core.Application.Common.Models.BaseResponse<FinTech.Core.Application.DTOs.ClientPortal.RecurringPaymentDto>.Failure("Not implemented"));
        }

        public Task<FinTech.Core.Application.Common.Models.BaseResponse<bool>> CancelRecurringPaymentAsync(Guid customerId, Guid paymentId)
        {
            return Task.FromResult(FinTech.Core.Application.Common.Models.BaseResponse<bool>.Failure("Not implemented"));
        }

        public async Task<FinTech.Core.Application.Common.Models.BaseResponse<List<FinTech.Core.Application.DTOs.ClientPortal.BillerDto>>> GetAvailableBillersAsync()
        {
            var billers = await _dbContext.Billers.OrderBy(b => b.Name).ToListAsync();
            var dtos = billers.Select(b => new FinTech.Core.Application.DTOs.ClientPortal.BillerDto
            {
                Id = b.Id,
                Name = b.Name,
                Category = b.Category,
            ...
            return FinTech.Core.Application.Common.Models.BaseResponse<List<FinTech.Core.Application.DTOs.ClientPortal.BillerDto>>.SuccessResponse(dtos);
        }

        public async Task<FinTech.Core.Application.Common.Models.BaseResponse<List<FinTech.Core.Application.DTOs.ClientPortal.SavedPayeeDto>>> GetSavedPayeesAsync(Guid customerId)
        {
            var payees = await _dbContext.SavedPayees.Where(p => p.CustomerId == customerId).ToListAsync();
            var dtos = payees.Select(p => new FinTech.Core.Application.DTOs.ClientPortal.SavedPayeeDto
            {
                Id = p.Id,
                BillerId = p.BillerId,
            ...
            return FinTech.Core.Application.Common.Models.BaseResponse<List<FinTech.Core.Application.DTOs.ClientPortal.SavedPayeeDto>>.SuccessResponse(dtos);
        }

        public async Task<FinTech.Core.Application.Common.Models.BaseResponse<FinTech.Core.Application.DTOs.ClientPortal.SavedPayeeDto>> CreateSavedPayeeAsync(Guid customerId, FinTech.Core.Application.DTOs.ClientPortal.SavedPayeeCreateDto payeeDto)
        {
            var payee = new FinTech.Core.Domain.Entities.ClientPortal.SavedPayee
            {
                CustomerId = customerId,
                BillerId = payeeDto.BillerId,
            ...
            _dbContext.SavedPayees.Add(payee);
            await _dbContext.SaveChangesAsync();
            var dto = new FinTech.Core.Application.DTOs.ClientPortal.SavedPayeeDto
            {
                Id = payee.Id,
                BillerId = payee.BillerId,
                PayeeName = payee.PayeeName,
                CustomerReferenceNumber = payee.CustomerReferenceNumber
            };
            return FinTech.Core.Application.Common.Models.BaseResponse<FinTech.Core.Application.DTOs.ClientPortal.SavedPayeeDto>.SuccessResponse(dto);
        }

        public async Task<FinTech.Core.Application.Common.Models.BaseResponse<FinTech.Core.Application.DTOs.ClientPortal.SavedPayeeDto>> UpdateSavedPayeeAsync(Guid customerId, Guid payeeId, FinTech.Core.Application.DTOs.ClientPortal.SavedPayeeUpdateDto payeeDto)
        {
            var payee = await _dbContext.SavedPayees.FindAsync(payeeId);
            if (payee == null || payee.CustomerId != customerId) return FinTech.Core.Application.Common.Models.BaseResponse<FinTech.Core.Application.DTOs.ClientPortal.SavedPayeeDto>.Failure("Not found");
            payee.PayeeName = payeeDto.PayeeName ?? payee.PayeeName;
            payee.CustomerReferenceNumber = payeeDto.CustomerReferenceNumber ?? payee.CustomerReferenceNumber;
        ...
        public async Task<FinTech.Core.Application.Common.Models.BaseResponse<bool>> DeleteSavedPayeeAsync(Guid customerId, Guid payeeId)
        {
            var payee = await _dbContext.SavedPayees.FindAsync(payeeId);
            if (payee == null || payee.CustomerId != customerId) return FinTech.Core.Application.Common.Models.BaseResponse<bool>.Failure("Not found");
            _dbContext.SavedPayees.Remove(payee);
            await _dbContext.SaveChangesAsync();
            return FinTech.Core.Application.Common.Models.BaseResponse<bool>.SuccessResponse(true);
        }
    }
