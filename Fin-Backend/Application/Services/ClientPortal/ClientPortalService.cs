...
namespace FinTech.Core.Application.Services
{
    public interface IClientPortalService
    {
        ...
        Task<IEnumerable<ClientPortalSession>> GetClientSessionsAsync(Guid profileId, int limit = 10);
        Task<ClientPortalSession> StartSessionAsync(Guid profileId, string ipAddress, string userAgent);
    ...
    public class ClientPortalService : IClientPortalService
    {
        ...
        public async Task<IEnumerable<ClientPortalSession>> GetClientSessionsAsync(Guid profileId, int limit = 10)
        {
            try
            {
                return await _dbContext.ClientPortalSessions
        ...
        public async Task<ClientPortalSession> StartSessionAsync(Guid profileId, string ipAddress, string userAgent)
        {
            try
            {
                var deviceInfo = ParseUserAgent(userAgent);

                var session = new ClientPortalSession
                {
                ...
                _dbContext.ClientPortalSessions.Add(session);
        ...
        public async Task EndSessionAsync(Guid sessionId)
        {
            try
            {
                var session = await _dbContext.ClientPortalSessions.FindAsync(sessionId);
                if (session != null)
                {
                    session.LogoutTime = DateTime.UtcNow;
                    session.IsActive = false;
                    session.LastModifiedOn = DateTime.UtcNow;
                    
                    _dbContext.ClientPortalSessions.Update(session);
                    await _dbContext.SaveChangesAsync();
                }
            }
            catch (Exception ex)
