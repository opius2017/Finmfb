...
namespace FinTech.Core.Application.Services.Accounting
{
    ...
    public class BudgetingService : IBudgetingService
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                // Id is auto-generated by BaseEntity
                Name = budgetDto.Name,
                Description = budgetDto.Description,
                FinancialPeriodId = Guid.Parse(budgetDto.FinancialPeriodId),
                Type = budgetDto.Type,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (budget.FinancialPeriodId.ToString() != financialPeriodId)
            {
            ...
            foreach (var budgetItem in budget.BudgetItems)
            {
                ...
                // Use default date of today if period not found (logic handled earlier)
                var accountBalances = await _generalLedgerService.GetAccountBalancesAsync(
                    new[] { budgetItem.AccountId }, 
                    financialPeriod.EndDate, // Pass date, not ID
                    cancellationToken);
                    
                decimal actualAmount = accountBalances.FirstOrDefault()?.Balance ?? 0;
                ...
                if (includeDetails)
                {
                    var accountVariance = new AccountVarianceDto
                    {
                        ...
                        IsFavorable = IsFavorableVariance(variance, account.Classification.ToString())
                    };
        ...
        private BudgetDto MapBudgetToDto(Budget budget)
        {
            ...
            var dto = new BudgetDto
            {
                ...
                FinancialPeriodId = budget.FinancialPeriodId.ToString(),
                Type = budget.Type,
                Status = budget.Status,
                CreatedBy = budget.CreatedBy,
                CreatedDate = budget.CreatedDate,
                LastModifiedBy = budget.LastModifiedBy,
                // FinTech Best Practice: LastModifiedDate is nullable DateTime, use null-coalescing
                LastModifiedDate = budget.LastModifiedDate ?? budget.CreatedDate,
